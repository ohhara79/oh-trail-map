 ...
 * only true of points — 281 rows instead of 36, which is why render() refuses to
...
export class PointsList {
  ...
  render(hidden: ReadonlySet<string>, selected: string | null): void {
    ...
    // changes what a row *says*, and rebuilding 281 rows to say it would throw the
   ...
   * enough to ask on every toggle: 281 string compares and no allocation, against
  ...
  private rebuild(matches: readonly PointRow[], tokens: string[]): void {
    ...
    for (const row of matches) {
      ...
      // Named as well as titled, unlike a trail row: 281 checkboxes announced as
